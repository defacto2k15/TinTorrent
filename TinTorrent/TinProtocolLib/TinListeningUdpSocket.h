...
public:
	...
	void initSocket(){
		...
		Assertions::check(bindResult != -1, "Binding socket in broadcast listener failed");
	}
	...
	std::pair<BroadcastMessage, TinAddress> listenForMessages(){
		...
		Assertions::check([&jsonString, this](){ return jsonString.length() < messagesBuffer.getSize();}, //todo change to equal
